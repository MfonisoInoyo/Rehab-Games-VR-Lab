...
public class KinectManager : MonoBehaviour
{
    ...
    private MoveForward movefScript;
    ...
    // gesture names 
    private string shiftLeftGestureName = "Shift_Left";
    private string shiftRightGestureName = "Shift_Right";
    private string moveForwardGestureName = "MoveForward";
    ...
    void Start()
    {
        ...
        movefScript = Player.GetComponent<MoveForward>();
    ...
    // when you have determined that a body is in front of the kinect then you want to determine
    // and that they are performing a certain gesture , then thi function translates the 
    // movement to a particular aspect in the game based on the level of correctness
    private void OnGestureDetected(object sender, GestureEventArgs e, int bodyIndex)
    {
        var isDetected = e.IsBodyTrackingIdValid && e.IsGestureDetected;
        
        if(e.GestureID == shiftLeftGestureName)
        {
        ...
        if (e.GestureID == shiftRightGestureName)
        {
        ...
        if (e.GestureID == moveForwardGestureName)
        {
            if (e.DetectionConfidence > 0.65f)
            {
                movefScript.forward = true;
            }
            else
            {
                movefScript.forward = false;
            }
        }
    }
